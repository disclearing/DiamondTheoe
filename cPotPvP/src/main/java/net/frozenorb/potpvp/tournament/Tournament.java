...
public class Tournament {
    ...
    public void check() {
        ...
        if (activeParties.size() == 1) {
            ...
            if (party.getMembers().size() == 1) {
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&b" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + " &7won the tournament!"), 4, 2);
                ...
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&b" + PatchedPlayerUtils.getFormattedName(members[0]) + " &7and &b" + PatchedPlayerUtils.getFormattedName(members[1]) + " &7won the tournament!"), 4, 2);
            } else {
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&b" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + "&7's team won the tournament!"), 4, 2);
            }
        ...
        if (--beginNextRoundIn >= 1) {
            ...
            case 1:
                if (currentRound == 0) {
                    Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&7The &btournament &7will begin in &b" + beginNextRoundIn + "&7."));
                } else {
                    Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&bRound " + (currentRound + 1) + " &7will start in &3" + beginNextRoundIn + "&7."));
                }
            }
    ...
    private void startRound() {
        ...
        Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&bRound " + ++currentRound + " &7has begun. Good luck!"));
    ...
    public void broadcastJoinMessage() {
        ...
        Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&7A &b&l" +  type.getDisplayName() + "&7 Tournament has started! Type &b/join&7 to play. &7(" + (this.activeParties.size() * multiplier) + "&7/" + (requiredTeams * multiplier) + "&7)"));
        Bukkit.broadcastMessage("");
    }
    ...
    private void broadcastJoinMessage(Party joiningParty) {
        if (getCurrentRound() != -1) {
            ...
            if (joiningParty.getMembers().size() == 1) {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&b&lDONOR ONLY &7- " + CoreAPI.INSTANCE.getProfile(joiningParty.getLeader()).getNameWithColor() + "&7 &7has &7joined &7the &btournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            } else if (joiningParty.getMembers().size() == 2) {
                Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&b&lDONOR ONLY &7- " + CoreAPI.INSTANCE.getProfile(membersIterator.next()).getNameWithColor() + "&7 &7and &b" + CoreAPI.INSTANCE.getProfile(membersIterator.next()).getNameWithColor() + "&7 have joined the &btournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
            } else {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&b&lDONOR ONLY &7- " + CoreAPI.INSTANCE.getProfile(joiningParty.getLeader()).getNameWithColor() + "&7's team has joined the &btournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            }

            message.tooltip(ChatColor.translateAlternateColorCodes('&', "&bDonators &7can join during the tournament countdown. Purchase a rank at &6 " + (PotPvPSI.getInstance().getDominantColor() == ChatColor.LIGHT_PURPLE ? "store.lunar.gg" : "store.lunar.gg") +  " &7."));
        ...
        FancyMessage message;
        if (joiningParty.getMembers().size() == 1) {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', Core.INSTANCE.getProfileManagement().getProfile(joiningParty.getLeader()).getNameWithColor() + "&7 has joined the &btournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        } else if (joiningParty.getMembers().size() == 2) {
            Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', CoreAPI.INSTANCE.getProfile(membersIterator.next()).getNameWithColor() + "&7 and &b" + CoreAPI.INSTANCE.getProfile(membersIterator.next()).getNameWithColor() + "&7 have joined the &btournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
        } else {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', Core.INSTANCE.getProfileManagement().getProfile(joiningParty.getLeader()).getNameWithColor() + "&7's team has joined the &btournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        }
