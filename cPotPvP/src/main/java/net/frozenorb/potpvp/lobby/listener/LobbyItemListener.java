...
import net.frozenorb.potpvp.party.command.PartyCreateCommand;
...
public final class LobbyItemListener extends ItemListener {
    ...
    public LobbyItemListener(LobbyHandler lobbyHandler) {
        ...
        addHandler(LobbyItems.EVENTS_ITEM, p -> {
            if (!p.hasPermission("core.command.event")) {
                if (GameQueue.INSTANCE.getCurrentGames().size() < 1) {
                    p.sendMessage("§cThere are no active events.");
                }
                p.sendMessage("§cYou can donate at store.crystalpvp.org for the ability to start an event!");
            } else {
                if (GameQueue.INSTANCE.getCurrentGames().size() > 0)
                    new EventsMenu().openMenu(p);
                else
                    new HostMenu().openMenu(p);
            }
        });
