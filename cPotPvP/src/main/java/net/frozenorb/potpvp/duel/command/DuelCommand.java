...
public final class DuelCommand {
    ...
    public static void duel(Player sender, @Param(name = "player") Player target) {
        ...
        if (senderParty != null && targetParty != null) {
            ...
            if (target.hasPermission("group.youtuber") && System.currentTimeMillis() - lobbyHandler.getLastLobbyTime(target) < 3_000) {
                sender.sendMessage(ChatColor.AQUA + target.getName() + " just returned to the lobby, please wait a moment.");
                return;
            }
    ...
    public static void duel(Player sender, Player target, KitType kitType) {
        ...
        if (alreadySentInvite != null) {
            if (alreadySentInvite.getKitType() == kitType) {
                sender.sendMessage(ChatColor.AQUA + "You have already invited " + ChatColor.DARK_GRAY + CoreAPI.INSTANCE.getProfile(target.getUniqueId()).getNameWithColor() + ChatColor.AQUA + " to a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel.");
                return;
            } else {
        ...
        target.sendMessage(ChatColor.DARK_AQUA + CoreAPI.INSTANCE.getProfile(sender.getUniqueId()).getNameWithColor() + ChatColor.AQUA + " has sent you a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel.");
        target.spigot().sendMessage(createInviteNotification(sender.getName()));

        sender.sendMessage(ChatColor.AQUA + "Successfully sent a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel invite to " + ChatColor.DARK_AQUA + CoreAPI.INSTANCE.getProfile(target.getUniqueId()).getNameWithColor() + ChatColor.AQUA + ".");
    ...
    public static void duel(Player sender, Party senderParty, Party targetParty, KitType kitType) {
        ...
        if (alreadySentInvite != null) {
            if (alreadySentInvite.getKitType() == kitType) {
                sender.sendMessage(ChatColor.AQUA + "You have already invited " + ChatColor.DARK_AQUA + targetPartyLeader + "'s party" + ChatColor.AQUA + " to a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel.");
                return;
            } else {
                // if an invite was already sent (with a different kit type)
                // just delete it (so /accept will accept the 'latest' invite)
        ...
        targetParty.message(ChatColor.DARK_AQUA +CoreAPI.INSTANCE.getProfile(sender.getUniqueId()).getNameWithColor() + "'s Party (" + senderParty.getMembers().size() + ")" + ChatColor.AQUA + " has sent you a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel.");
        Bukkit.getPlayer(targetParty.getLeader()).spigot().sendMessage(createInviteNotification(sender.getName()));

        sender.sendMessage(ChatColor.AQUA + "Successfully sent a " + kitType.getColoredDisplayName() + ChatColor.AQUA + " duel invite to " + ChatColor.DARK_AQUA + targetPartyLeader + ChatColor.AQUA + "'s party" + ".");
    ...
    private static TextComponent[] createInviteNotification(String sender) {
        ...
        firstPart.setColor(net.md_5.bungee.api.ChatColor.AQUA);
        commandPart.setColor(net.md_5.bungee.api.ChatColor.DARK_AQUA);
        secondPart.setColor(net.md_5.bungee.api.ChatColor.AQUA);
