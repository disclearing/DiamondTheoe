...
final class MatchScoreGetter implements BiConsumer<Player, LinkedList<String>> {
    ...
    public void accept(Player player, LinkedList<String> scores) {
        ...
        if (match == null) {
            if (followingOpt.isPresent()) {
                scores.add("&bFollowing: *&f" + CoreAPI.INSTANCE.getProfile(followingOpt.get()).getNameWithColor());
            }
    ...
    private void render1v1MatchLines(List<String> scores, MatchTeam otherTeam) {
        scores.add("&bOpponent: &f" + FrozenUUIDCache.name(otherTeam.getFirstMember()));
    }
    ...
    private void renderSpectatorLines(List<String> scores, Match match, MatchTeam oldTeam) {
        ...
        scores.add("&bKit: &f" + match.getKitType().getColoredDisplayName() + rankedStr);
    ...
    private void renderMetaLines(List<String> scores, Match match, boolean participant) {
        ...
        scores.add(PotPvPSI.getInstance().getDominantColor() + "&bDuration: &f" + formattedDuration);
    }
    ...
    private void renderPingLines(List<String> scores, Match match, Player ourPlayer) {
        ...
        if (teams.size() == 2) {
            ...
            if (firstTeamPlayers.size() == 1 && secondTeamPlayers.size() == 1) {
                scores.add("&b"); // spaceer
                scores.add("&bYour Ping: &f" + PlayerUtils.getPing(ourPlayer));
                ...
                scores.add("&bTheir Ping: &f" + PlayerUtils.getPing(otherPlayer));
            }
        }
    }
