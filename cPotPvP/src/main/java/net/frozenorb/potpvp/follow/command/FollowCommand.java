...
public final class FollowCommand {
    ...
    public static void follow(Player sender, @Param(name = "target") Player target) {
        ...
        if (sender == target) {
            ...
            if (sender.isOp()) {
                sender.sendMessage(ChatColor.RED + "Bypassing " + CoreAPI.INSTANCE.getProfile(target.getUniqueId()).getNameWithColor() + ChatColor.RED + "'s no spectators preference...");
            } else {
                sender.sendMessage(ChatColor.RED + CoreAPI.INSTANCE.getProfile(target.getUniqueId()).getNameWithColor() + ChatColor.RED + " doesn't allow spectators at the moment.");
                return;
            }
        }
