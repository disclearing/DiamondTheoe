...
public final class FollowHandler {
    ...
    public void startFollowing(Player player, Player target) {
        ...
        player.sendMessage(ChatColor.AQUA + "Now following " + ChatColor.DARK_AQUA + CoreAPI.INSTANCE.getProfile(target.getUniqueId()).getNameWithColor() + ChatColor.AQUA + ", exit with /unfollow.");
    ...
    public void stopFollowing(Player player) {
        ...
        if (prevTarget != null) {
            player.sendMessage(ChatColor.AQUA + "Stopped following " + ChatColor.DARK_AQUA + CoreAPI.INSTANCE.getProfile(prevTarget).getNameWithColor() + ChatColor.AQUA + ".");
            InventoryUtils.resetInventoryDelayed(player);
            VisibilityUtils.updateVisibility(player);
        }
    }
