...
public class StatsResetCommands {
    ...
    public static void addToken(CommandSender sender, @Param(name = "player") String playerName, @Param(name = "amount") int amount) {
        ...
        Core.INSTANCE.getProfileManagement().loadProfile(uuid);
        sender.sendMessage(ChatColor.GREEN + "Added " + amount + " token" + (amount == 1 ? "" : "s") + " to " + CoreAPI.INSTANCE.getProfile(uuid).getNameWithColor() + ChatColor.GREEN + ".");
    }
