...
final class PostMatchHealsLeftButton extends Button {
    ...
    public List<String> getDescription(Player player) {
        return ImmutableList.of(
            ChatColor.AQUA + CoreAPI.INSTANCE.getProfile(this.player).getNameWithColor() + ChatColor.AQUA + " had " + healsRemaining + " " + (healsRemaining == 1 ? healingMethod.getLongSingular() : healingMethod.getLongPlural()) + " left.",
            ChatColor.AQUA + CoreAPI.INSTANCE.getProfile(this.player).getNameWithColor() + ChatColor.AQUA + " missed " + missedHeals + " health potion" + (missedHeals == 1 ? "." : "s.")
        );
    }
