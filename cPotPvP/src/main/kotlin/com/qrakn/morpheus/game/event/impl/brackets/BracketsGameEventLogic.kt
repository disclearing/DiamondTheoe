...
open class BracketsGameEventLogic(val game: Game) : GameTeamEventLogic(game) {
    ...
    fun check() {
        ...
        if (getNextParticipant(winner) == null) {
            ...
            game.sendMessage("", winner.getName() + org.bukkit.ChatColor.AQUA.toString() + " beat " + loser.getName() + ChatColor.AQUA + "!", "")
            next()
        }
    }
    ...
    private fun next() {
        ...
        if (fighter != opponent && fighter != null && opponent != null) {
            ...
            game.sendMessage("", ChatColor.AQUA.toString() + "" + ChatColor.BOLD + "Next Matchup:", fighter.getName() + ChatColor.AQUA.toString() + " vs. " + opponent.getName() + ChatColor.AQUA + "!", "")
