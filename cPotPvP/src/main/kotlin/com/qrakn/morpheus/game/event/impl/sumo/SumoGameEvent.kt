...
object SumoGameEvent : GameEvent {
    ...
    private const val PERMISSION = "core.command.host.sumo"
    ...
    override fun getScoreboardScores(player: Player, game: Game): List<String> {
        ...
        toReturn.add("&3Event &7($name)")
        toReturn.add("&b ${PotPvPLang.LEFT_ARROW_NAKED} &fPlayers: &7${logic.getPlayersLeft()}/${game.players.size}")

        if (game.state == GameState.RUNNING) {
            toReturn.add("&b ${PotPvPLang.LEFT_ARROW_NAKED} &fRound: &7${logic.getRound()}")
            if (game.getParameter(GameTeamSizeParameter.Duos.javaClass) == null) {
